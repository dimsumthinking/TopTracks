...
extension StationFiller {
  ...
  static func rotation(for musicTestSongs: MusicTestSongs) async -> [SongInCategory] {
    let power = musicTestSongs.testResults.filter{$0.rotationCategory == .power}.map(\.song)
